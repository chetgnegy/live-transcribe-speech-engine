...
might meet challenges building the c++ Opus library. Fortunately you can get 
...
That doesn't mean the gradle files won't try to build Opus, but your code no 
longer strictly requires it. Go through the gradle files and comment out any 
references to the c++ libs and [the static import of the Opus lib][https://github.com/google/live-transcribe-speech-engine/blob/master/app/src/main/java/com/google/audio/StreamingAudioEncoder.java#L541]. 

For development and local testing, Opus vs. uncompressed audio doesn't matter 
much. It's enough to decide whether these libraries suit your needs, so you 
